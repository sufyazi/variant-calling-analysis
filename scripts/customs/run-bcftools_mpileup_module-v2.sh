...
if [ -f "${outfile}"/"${data_id}"_"${motif_id}"_qualgt10.var.flt.vcf ]; then
    ...
    if [ -n "$bed_path" ]; then
        ...
        if bcftools mpileup -Ou -f /scratch/users/ntu/suffiazi/inputs/references/gatk4/GRCh38_no_alt_analysis_set_GCA_000001405.15.fasta -T "${bed_path}" -b "${bams}" --annotate FORMAT/AD,FORMAT/DP | bcftools call -Ou -mv | bcftools filter -i'QUAL>10' | bcftools +fill-tags - -- -t AF > "${outfile}"/"${data_id}"/"${data_id}"_"${motif_id}"_qualgt10.var.flt.vcf; then
